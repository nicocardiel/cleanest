...
C Variables locales
        ...
        REAL DX,DY
        CHARACTER*255 BUTT_LABEL
C------------------------------------------------------------------------------
C------------------------------------------------------------------------------
        IF(MODOTEXT_BUTT.OR.MODOTEXT_HTML)THEN
          ...
          ELSE
            READ(*,*) NB
            READ(*,101) BUTT_LABEL
            WRITE(*,101) '<TABLE BORDER="5">'
            WRITE(*,100) '<TR><TD BGCOLOR="#AAAAAA"><B>'
            WRITE(*,100) BUTT_LABEL(1:TLENBUTT(BUTT_LABEL))
            WRITE(*,101) '</B></TD></TR>'
            WRITE(*,101) '</TABLE>'
          END IF
          IF(NB.EQ.0) RETURN
